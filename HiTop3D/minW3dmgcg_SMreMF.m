...
%%%% With sensitivity monitoring and matrix-free approximate reanalysis %%%%
...
% To reproduce the result tagged "MinV-SMMFRE5" in Table 2 of the paper, run:
% minW3dmgcg_SMreMF(48,24,24,3,sqrt(3),1,4,1e0,100,3330,'minW3dmgcg_SMreMF')
function minW3dmgcg_SMreMF(nelx,nely,nelz,penal,rmin,ft,nl,cgtol,cgmax,compconst,filename)
tic;
%% MATERIAL PROPERTIES
E0 = 1;
Emin = 1e-9;
...
while (change > 1e-2 && loop < 50)
    loop = loop+1;
    %% FE-ANALYSIS
    if (count == 49 || mgcgr > 1e-2)
        Kpre = cell(nl,1);
        sK = reshape(KE(:)*(Emin+xPhys(:)'.^penal*(E0-Emin)),576*nelem,1);
        Kpre{1,1} = sparse(iK,jK,sK);
        Kpre{1,1} = Null'*Kpre{1,1}*Null - (Null-speye(ndof,ndof));
        for l = 1:nl-1
            Kpre{l+1,1} = Pu{l,1}'*(Kpre{l,1}*Pu{l,1});
        end
        Lfac = chol(Kpre{nl,1},'lower'); Ufac = Lfac';
        [mgcgi,mgcgr,U] = mgcg(Kpre{1,1},F,U,Kpre,Lfac,Ufac,Pu,nl,1,cgtol,cgmax,...
            ceil(2^(nl-2)-0.5-rmin/sqrt(3)),KE,edofMat,Emin,E0,xPhys,penal,kktcheck); % These are extras for computing sens-mon
        count = 0;
        assems = assems + 1;
    else
        [mgcgi,mgcgr,U] = mgcgMatrixFree(F,U,Kpre,Lfac,Ufac,Pu,nl,1,1e-3,5,...
            KE,edofMat,Emin,E0,xPhys,penal,Null); 
        count = count + 1;
    end
    ...
    %% Keep stats
    Stats(loop,:) = [f0val fval mgcgi mgcgr max(kktcheck) assems];
end
time = toc;
%% Save results
save(filename,'Stats','time','xPhys');
%% VOXEL EXPORT (binarized, [y,z,x] = [nely,nelz,nelx])
isoThresh = 0.8;  % keep in sync with the plot's isosurface
voxBin = reshape(xPhys, [nely, nelz, nelx]) >= isoThresh;  % [y,z,x]
save([filename '_voxBin.mat'], 'voxBin', '-v7');
%% PLOT
% % For double symmetry
...
%% FUNCTION mgcgMatrixFree - MULTIGRID PRECONDITIONED CONJUGATE GRADIENTS
function [i,relres,u] = mgcgMatrixFree(b,u,Apre,Lfac,Ufac,Pu,nl,nswp,tol,maxiter,...
    KE,edofMat,Emin,E0,xPhys,penal,Null)
xPhys24 = repmat(xPhys(:),1,24);
Evals = Emin + xPhys24.^penal*(E0-Emin);
Auvals = u(edofMat)*KE;
Auvals = Evals.*Auvals;
